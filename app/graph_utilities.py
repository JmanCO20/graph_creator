...
def creating_trendlines(trendlines: dict[str, bool], checkboxes: dict[str, bool | float], ax: plt.Axes, x, y, upper_bound, lower_bound, half_of_upper: int | None=None, half_of_lower: int | None=None):
    try:
        if not checkboxes["has_y_int"]:
            raise ValueError
        if trendlines["average"]:
            x_line, y_line, m = create_average_line(checkboxes["has_y_int"], checkboxes["y_int"], x, y)
            st.session_state.previous_lines["average"] = None
            ax.plot(x_line, y_line, color="blue", label=f"y = {m:.3g}x + {checkboxes["y_int"]:.3g}")
        if trendlines["upper"]:
            x_line, y_line, m = create_average_line(checkboxes["has_y_int"], checkboxes["y_int"], x, upper_bound)
            st.session_state.previous_lines["upper"] = None
            ax.plot(x_line, y_line, color="orange", label=f"y = {m:.3g}x + {checkboxes["y_int"]:.3g}")
        if trendlines["lower"]:
            x_line, y_line, m = create_average_line(checkboxes["has_y_int"], checkboxes["y_int"], x, lower_bound)
    ...
    try:
        if checkboxes["has_y_int"]:
            raise ValueError
        if trendlines["average"]:
            x_line, y_line, m, b = create_average_line(checkboxes["has_y_int"], checkboxes["y_int"], x, y)
            st.session_state.previous_lines["average"] = None
...
def draw_lines(previous_lines: dict[str, tuple], ax: plt.Axes, x: np.ndarray):
...
def load_user_graph(df, labels: dict[str, str], previous_lines: dict[str, tuple], checkboxes: dict[str, bool | float], trendlines: dict[str, bool], window_size: dict[str, float]):
    ...
    draw_lines(previous_lines, ax, x)
...
def create_graph_from_user(df, labels: dict[str, str], wants_legend: bool, graph_attributes: dict[str, float], trendlines: dict[str, bool], checkboxes: dict[str, bool | float], window_size: dict[str, float]):
    ...
    if st.session_state.upper_enter:
        ...
        draw_lines(st.session_state.previous_lines, ax, x)
